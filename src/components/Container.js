...
function mapStateToProps({ phones }) {
  const inCart_phones = Object.keys(phones).filter(
    (phone) => phones[phone].inCart === "true"
  );
  const outCart_phones = Object.keys(phones).filter(
    (phone) => !inCart_phones.includes(phone)
  );
